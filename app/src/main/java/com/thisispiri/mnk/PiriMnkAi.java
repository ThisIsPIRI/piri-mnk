...
public class PiriMnkAi extends MnkAi {
	...
	private void newEvaluate(final int x, final int y) {
		value = new CellValue(valueLength);
		if (game.array[y][x] == Shape.O || game.array[y][x] == Shape.X) { //if the cell is already filled, it has no importance.
			value.ownLines[valueLength - 1] = -1;
			return;
		}
		newExamineLine(x, y, -1, 0); // - shape
		newExamineLine(x, y, 0, -1); // | shape
		newExamineLine(x, y, -1, 1); // / shape
		newExamineLine(x, y, -1, -1); // \ shape
	}
	private void newExamineLine(final int x, final int y, final int xP, final int yP) {
		boolean isOpen = true, isOppositeOpen = false;
		Shape lineShape = null;
		int streak = 0, emptyCells = 0, innerBlank = 0, blank;
		int i, j;
		for(i = y + yP, j = x + xP;inBoundary(i, j);i += yP, j += xP) {
			if(game.array[i][j] == EMPTY) emptyCells++;
			else if(lineShape == null) lineShape = game.array[i][j];
			else if(game.array[i][j] != lineShape) {
				isOpen = false;
				break;
			}
			if(lineShape == game.array[i][j]) {
				if(STREAK_SCALE > emptyCells) { //only travel through empty cells if it is favorable
					streak++;
					innerBlank += emptyCells;
					emptyCells = 0;
				}
				else break;
			}
			if(Math.abs(i - y) >= game.winStreak) break;
		}
		blank = lineSpaces(j + xP, i + yP, x, y, xP, yP);
		int previousStreak = streak;
		if(streak + blank < game.winStreak) streak = 0;
		if(inBoundary(y - yP, x - xP) && game.array[y - yP][x - xP] == game.empty && lineShape != game.empty)
			isOppositeOpen = true;
		if(!inBoundary(i + yP, j + xP) || game.array[i + yP][j + xP] != EMPTY) isOpen = false;
		newUpdate(streak, innerBlank, blank, lineShape, isOpen, isOppositeOpen);

		Shape previousShape = lineShape;
		boolean isConnected = false;
		lineShape = null; emptyCells = 0; streak = 0; innerBlank = 0; isOpen = true; isOppositeOpen = false;
		for(i = y - yP, j = x - xP;inBoundary(i, j);i -= yP, j -= xP) {
			if(game.array[i][j] == EMPTY) emptyCells++;
			else if(lineShape == null) {
				lineShape = game.array[i][j];
				if(lineShape == previousShape) {
					isConnected = true;
		...
		if(previousStreak > 0) previousStreak++; //for blank check. x + (previousStreak * xP) is on the end of the line, not out of it
		blank = lineSpaces(j - xP, i - yP, x + (previousStreak * xP), y + (previousStreak * yP), xP, yP);
		if(streak + blank < game.winStreak) streak = 0;
		if(inBoundary(y + yP, x + xP) && game.array[y + yP][x + xP] == EMPTY && lineShape != EMPTY)
			isOppositeOpen = true;
		if(!inBoundary(i - yP, j - xP) || game.array[i - yP][j - xP] != EMPTY) isOpen = false;
		newUpdate(streak, innerBlank, blank, lineShape, isOpen, isOppositeOpen || isConnected);
	}
	private void newUpdate(int streak, final int innerBlank, final int outerBlank, final Shape lineShape, final boolean isOpen, final boolean isConnectedOrOppositeOpen) {
		streak *= STREAK_SCALE;
		int[] target;
		if(game.shapes[game.nextShape] == lineShape) target = value.ownLines;
		else target = value.enemyLines;
		if(isConnectedOrOppositeOpen) streak += OPPOSITE_OPEN_BONUS;
		if (isOpen) {
			streak += OPEN_BONUS / (outerBlank >= FULL_OPEN_BONUS_THRESHOLD ? 1 : 2);
			if(target[streak] == 1) { //if the cell can block two lines of x cells, mark it as an open line of x + DOUBLE_OPEN_BONUS cells.
				...
				target[streak + DOUBLE_OPEN_BONUS - innerBlank <= 0 ? 0 : streak + DOUBLE_OPEN_BONUS - innerBlank]++;
				return;
			}
		}
		if(streak >= target.length) streak = target.length - 1;
		target[streak - innerBlank <= 0 ? 0 : streak - innerBlank]++;
	}*/
